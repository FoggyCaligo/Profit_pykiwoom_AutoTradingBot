...
class Main(QMainWindow):
    ...
    def _handler_real_data(self,code,real_type,data):
        ...
        if real_type == "장시작시간":#장 시작 전--------------------------------
            ...
            # print(data)
            # print('\n')
            # self.currPricedict[code] = self.get_each_stock_data(code)
            # self.expecPriceDict[code] = logic.calc_assumePriceIndex(int(self.currPricedict[code]))
            # curr_price_idx = len(self.currPricedict)/2
            # curr_price = self.currPricedict[code][curr_price_idx]
            # if self.expecPriceDict[code] > curr_price + curr_price*2/1000 + curr_price*15/10000 + curr_price*2/100:#예상가 > 현재가+세금(0.2%)+수수료(0.015%)+2%수익
