...
class Main(QMainWindow):
    ...
    def __init__(self):
        ...
        self.ocx.OnReceiveTrData.connect(self.update_stockdata)
...
#순서대로
    ...
    def update_stockdata(self, sTrCode, sPrevNext):
        sPrevNext = '0'
        sScrNo = 1000
        sRQName = "계좌평가잔고내역요청"

        if True:
            # total_buy_money = self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)",
            #                                     sTrCode, sRQName, 0, "총매입금액")
            # self.total_buy_money = int(total_buy_money)
            # total_profit_loss_money = self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)",
            #                                     sTrCode, sRQName, 0, "총평가손익금액")
            # self.total_profit_loss_money = int(total_profit_loss_money)
            # total_profit_loss_rate = self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)",
            #                                     sTrCode, sRQName, 0, "총수익률(%)")
            # self.total_profit_loss_rate = float(total_profit_loss_rate)

            # print("계좌평가잔고내역요청 싱글데이터 : %s - %s - %s" % (total_buy_money,
            #                                    total_profit_loss_money, total_profit_loss_rate))

#            self.stop_screen_cancel(self.screen_my_info)
            # 책에는 있지만 깃헙에는 없는 내용

            rows = self.ocx.dynamicCall("GetRepeatCnt(QString, QString)", sTrCode, sRQName)
            ...
            for i in range(rows):
                ...
                code_nm = self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)",
                                           sTrCode, sRQName, i, "종목명")
                stock_quantity = self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)",
                                           sTrCode, sRQName, i, "보유수량")
                buy_price = self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)",
                                           sTrCode, sRQName, i, "매입가")  # 매입가 평균
                learn_rate = self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)",
                                           sTrCode, sRQName, i, "수익률(%)")
                current_price = self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)",
                                           sTrCode, sRQName, i, "현재가")
                total_chegual_price = self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)",
                                           sTrCode, sRQName, i, "매입금액")
                possible_quantity = self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)",
                ...
                self.haveamounts[code] = stock_quantity
                self.currprice[code] = current_price
...
#기타 필요한 함수들----------------
    ...
    def sell_all(self):
        ...
        for each in self.codes:
            self.sell(each,self.haveamounts[each],self.buyprices[each])
            # self.buyprices[each] = 0
            # self.predprices[each]=0
            # self.haveamounts[each]=0
            # self.remainamounts[each]=0
