...
def getdict():
    for each in ks100['Code']:
        each = str(each)
        currPricedf = (fdr.DataReader(symbol=each,start=str(dt.date.today())))
        currP = int(currPricedf['Open'].values)
        if(10000<currP and currP<50000):
            currPrice_dict[each]=currP
